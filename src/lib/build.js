...
function shiftRate(rate, by) {
  ...
  const next = Math.max(-45, Math.min(20, base + by));
  return `${next >= 0 ? '+' : ''}${next}%`;
}
...
function buildVocabulary(plan) {
  ...
  words.forEach((w, i) => {
    const imgId = plan.image(`word-${i + 1}`, w.image_prompt || `${w.word}, ${lesson.topic}`, lesson.topic);
    ...
    plan.say(`${w.word}.`, { rate: shiftRate(lvl.rate, -14), note, pauseAfterMs: 620 });
...
function buildReading(plan) {
  ...
  plan.scene('passage', {
    ...
    note: 'Follow the highlighted words. Read them in your head as you hear them.',
    imageId: heroId,
  });
  lesson.passage.forEach((s, i) => {
    plan.say(s.en, { ar: s.ar, note: `Reading · ${i + 1} of ${lesson.passage.length}` });
  });

  if (lesson.glossary?.length) {
    plan.scene('title', {
      eyebrow: 'Words from the text',
      title: 'Six words worth keeping',
      subtitle: lesson.passage_title || lesson.topic,
      chips: lesson.glossary.map(g => g.word),
    });
    plan.say('Before the questions, here are the words from the text that are worth remembering.');
    ...
    lesson.glossary.forEach((g, i) => {
      ...
      plan.say(`${g.word}.`, { rate: shiftRate(lvl.rate, -12), pauseAfterMs: 560, note: 'Glossary' });
      plan.say(g.meaning, { ar: g.meaning_ar, note: 'Glossary' });
    });
  }
...
function buildListening(plan) {
  ...
  if (lesson.key_phrases?.length) {
    ...
    lesson.key_phrases.forEach((p) => {
      plan.say(`${p.phrase}.`, { rate: shiftRate(lvl.rate, -12), note: 'Key phrases', pauseAfterMs: 620 });
...
function buildSpeaking(plan) {
  ...
  lesson.drills.forEach((d, i) => {
    ...
    plan.say(`${d.phrase}`, { rate: shiftRate(lvl.rate, -16), ar: d.phrase_ar, note, pauseAfterMs: 420 });
    plan.say('Your turn.', {
      note,
